...
class PointAndDocumentControllerTest
{
    ...
    {
        when(documentAccessService.authorizeOriginalFile(11L, 22L, "127.0.0.1"))
                .thenReturn(new FileAuthorization("质量管理手册.docx",
                        "https://temporary.example/file", 300L));

        mockMvc.perform(post("/wx/documents/22/original").with(request -> {
        ...
        assertEquals(Arrays.asList(Long.class, HttpServletRequest.class),
