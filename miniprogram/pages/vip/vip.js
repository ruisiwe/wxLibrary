...
Page({
  ...
  load() {
    ...
    Promise.all([vip.plans(), vip.profile()]).then(([plans, profile]) => this.setData({ plans: plans || [], profile, loading: false }))
  ...
  buy(event) {
    ...
    vip.createOrder(planId).then(order => {
      ...
      return new Promise((resolve, reject) => wx.requestPayment({
        timeStamp: order.timeStamp, nonceStr: order.nonceStr, package: order.package,
        signType: order.signType, paySign: order.paySign, success: resolve, fail: reject
      }));
    }).then(() => {
