...
Page({
  ...
  load() {
    ...
    documents.detail(this.data.id).then(document => {
      this.setData({
        document,
        vipFreeDocument: this.isVipFreeDocument(document),
        canSendOriginal: documents.canSendOriginal({
          unlocked: false,
          pointPrice: document.pointPrice,
      ...
      return Promise.all([documents.unlocked(), documents.favorites(), vip.profile()]).then(([unlocked, favorites, profile]) => {
        ...
        const isUnlocked = matches(unlocked);
        const vipActive = Boolean(profile.vipActive);
        this.setData({
          unlocked: isUnlocked,
          favorite: matches(favorites),
          vipActive,
          canSendOriginal: documents.canSendOriginal({
            unlocked: isUnlocked,
            pointPrice: document.pointPrice,
  ...
  openFirstLoginForAction() {
    request({
        url: '/wx/public/agreements/current',
        protected: false
      })
  ...
  submitLogin(event) {
    ...
    if (!this.data.profileRequired) {
      return request({
        url: '/wx/agreements/accept',
        method: 'POST',
        data: {
          privacyAccepted: true,
          privacyVersion: privacy && privacy.version
        }
      }).then(() => {
    ...
    wx.login({
      success: result => auth.firstLogin({
        ...
        avatarPath: event.detail.avatarPath
      }).then(state => {
  ...
  preview() {
    ...
    wx.navigateTo({
      url: `/pages/document-reader/document-reader?id=${this.data.id}`
    });
  },
  unlock() {
    ...
    wx.showModal({
      title: vipFree ? '会员免费下载' : '积分兑换',
      content: vipFree ? '确认免费下载该会员免费文档？' : `确认使用 ${pointPrice} 积分永久兑换该文档？`,
      success: result => {
  ...
  toggleFavorite() {
    ...
    operation(this.data.id).then(result => this.setData({
  ...
  shareAvailableOriginal() {
    if (!this.requireLogin('shareAvailableOriginal')) return;
    if (this.data.unlocked) return this.shareOriginal();
    if (this.data.sendingOriginal) return;
    this.setData({
      sendingOriginal: true
    });
    const requestId = `unlock-${this.data.id}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    return documents.unlock(this.data.id, requestId, {
      freeOnly: true
    }).then(() => {
      this.setData({
        unlocked: true,
  ...
  shareOriginal() {
    ...
    documents.fileDisclaimer().then(disclaimer => {
      ...
      title: error.message || '免责声明加载失败，请重试',
      icon: 'none'
    }));
  },
  onSuppressReminderChange(event) {
    this.setData({
      suppressReminder: (event.detail.value || []).includes('suppress')
    });
  },
  cancelDisclaimer() {
    this.setData({
  ...
  sendOriginal(disclaimer, confirmed, reminderSuppressed) {
    ...
    return documents.original(this.data.id, {
      ...
      .then(filePath => this.shareFileAndRecord(filePath))
      .catch(error => wx.showToast({
