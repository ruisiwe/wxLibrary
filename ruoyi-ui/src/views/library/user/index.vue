<template>
  <div class="app-container">
    ...
    <el-dialog title="人工调整积分" :visible.sync="visible" width="520px">
      <el-alert title="扣减积分不能使余额小于 0，业务编号用于防止重复入账。" type="warning" :closable="false" />
      <el-form :model="form" label-width="100px" class="mt16">
        ...
        <el-form-item label="业务编号" required><el-input v-model="form.bizNo" maxlength="80" /></el-form-item>
        <el-form-item label="调整原因" required><el-input v-model="form.description" type="textarea" maxlength="200" /></el-form-item>
      </el-form>
      <span slot="footer"><el-button @click="visible=false">取消</el-button><el-button type="primary" @click="submitPoints">确认调整</el-button></span>
    </el-dialog>
...
export default {
  name: 'LibraryWxUser',
  data() { return { loading: false, rows: [], total: 0, query: { pageNum: 1, pageSize: 10 }, visible: false, detailVisible: false, detail: {}, selected: null, form: {} } },
  created() { this.load() },
  methods: {
    ...
    openPoints(row) { this.selected = row; this.form = { amount: 0, bizNo: '', description: '' }; this.visible = true },
    submitPoints() {
      if (!this.form.amount || !this.form.bizNo || !this.form.description) return this.$modal.msgError('请完整填写调整数量、业务编号和调整原因')
      this.$modal.confirm(`确认将用户积分调整 ${this.form.amount} 吗？`).then(() => adjustUserPoints(this.selected.id, this.form)).then(() => { this.$modal.msgSuccess('积分调整成功'); this.visible = false; this.load() })
    }
  }
}
</script>
