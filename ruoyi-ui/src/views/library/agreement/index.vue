<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="rows" border class="mt16">
      <el-table-column prop="agreementType" label="协议类型"><template slot-scope="s">{{s.row.agreementType==='PRIVACY'?'用户隐私协议':'网站声明'}}</template></el-table-column>
      <el-table-column prop="version" label="版本"/><el-table-column prop="title" label="标题" min-width="180"/><el-table-column prop="effectiveTime" label="生效时间" min-width="170"/><el-table-column prop="status" label="状态"/>
      <el-table-column label="操作" width="160"><template slot-scope="s"><el-button v-hasPermi="['library:agreement:edit']" type="text" @click="open(s.row)">修改</el-button><el-button v-if="s.row.status!=='PUBLISHED'" v-hasPermi="['library:agreement:publish']" type="text" @click="publish(s.row)">发布</el-button></template></el-table-column>
    </el-table>
    <el-dialog :title="form.id?'修改协议版本':'新增协议版本'" :visible.sync="visible" width="760px">
      <el-form :model="form" label-width="100px"><el-form-item label="协议类型" required><el-radio-group v-model="form.agreementType"><el-radio label="PRIVACY">用户隐私协议</el-radio><el-radio label="STATEMENT">网站声明</el-radio></el-radio-group></el-form-item><el-form-item label="版本" required><el-input v-model="form.version"/></el-form-item><el-form-item label="标题" required><el-input v-model="form.title"/></el-form-item><el-form-item label="协议内容" required><el-input v-model="form.content" type="textarea" :rows="12"/></el-form-item><el-form-item label="生效时间" required><el-date-picker v-model="form.effectiveTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss"/></el-form-item></el-form>
